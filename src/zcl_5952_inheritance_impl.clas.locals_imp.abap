...
*   declare a table - do not allow the same attribute to be used more than once
           tt_atributes TYPE SORTED TABLE OF ts_attributes WITH UNIQUE KEY name.

    METHODS constructor IMPORTING iv_manufacturer TYPE string
                                  iv_type         TYPE string.
    METHODS: get_attributes RETURNING VALUE(rt_attributes) TYPE tt_atributes.
